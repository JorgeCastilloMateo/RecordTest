...
change.point(ZaragozaSeries, weights = function(t) sqrt(t), record = "d", simulate.p.value = TRUE, B = 10000)
```

The point of change is found at time 36 (1986). We can see analogous results for the annual mean temperature series, a change-point is significantly detected with time estimate 38 (1988).

```{r}
test.result <- change.point(rowMeans(TxZ365))
test.result
```

```{r}
records(rowMeans(TxZ365)) + 
